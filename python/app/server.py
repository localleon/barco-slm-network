...
def sessionlogger(request):
    """
    This function appends every request with some useful informations as a string to an array. 
    The array is used as a temporary log. 
    """
    if(len(sessionlog) >= 100):
        del sessionlog[0:10]
    sessionlog.append(str('{:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()) + " : " +
                          request.remote_addr + " " +
                          request.method + " " +
                          request.scheme + " " +
                          request.full_path
